...
| `a` | Approve plan (saves and exits) |
...
`approved`, `changes_requested`, or `dismissed`; change requests also include `feedback`. An
integrating agent can watch this file to be notified when the TUI exits.
...
watch the result file so approvals and change requests return to the agent automatically.
