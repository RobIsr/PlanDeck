...
function ListView({
  ...
  useInput((input, key) => {
    ...
    else if (input === 'x' && plan.steps.length > 0) onExecuteStep();
    else if (input === '?' && plan.steps.length > 0) onAskStep();
...
function StepQuestionView({ step, onSubmit, onCancel }) {
  const [text, setText] = useState('');
  ...
  return h(Box, { flexDirection: 'column' },
    h(Box, { marginBottom: 1 },
      h(Text, { bold: true, color: 'blue' }, `Ask about: ${trunc(step.title, 60)}`),
    ),
    h(Box, { marginBottom: 1 },
      h(Text, null, 'What would you like explained?'),
    ),
    h(TextInput, { value: text, onChange: setText, focus: true }),
    h(HelpBar, { items: [['Enter', 'open question session'], ['Esc', 'cancel']] }),
  );
}
...
function App({ initialPlan, resultRef }) {
  ...
  const [selectedIndex, setSelectedIndex] = useState(() => {
    const pendingIndex = (initialPlan.steps || []).findIndex(step => step.status === 'pending');
    return pendingIndex === -1 ? 0 : pendingIndex;
  });
  ...
  function executeStep() {
    const step = { ...plan.steps[selectedIndex], status: 'in_progress' };
    const updatedPlan = {
      ...plan,
      steps: plan.steps.map((candidate, index) => index === selectedIndex ? step : candidate),
    };
    resultRef.current = {
      approved: false,
      executeStep: step,
  ...
  if (view === 'question' && plan.steps[selectedIndex]) {
    return h(StepQuestionView, {
      step: plan.steps[selectedIndex],
      onSubmit: askStep,
      onCancel: () => setView('list'),
    });
